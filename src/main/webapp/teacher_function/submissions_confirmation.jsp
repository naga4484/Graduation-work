...
<div class="class_submission_search">
	<form action="Submissions_class.action">
		<label>クラス</label> <select name="class_id">
...
<div class="submission_check">
	<c:if test="${distinctsubmissions_class != null}">
		...
		<table>
			...
			<tr>
				<c:forEach var="item" items="${distinctsubmissions_class}"
					varStatus="status">
					<td>${status.index + 1}</td>
					<td><a
						href="Submissions_detail.action?submissions_id=${item.submissions_id}">${item.submissions_id}</a></td>
					<td><a
						href="Submissions_detail.action?submissions_id=${item.submissions_id}">${item.name}</a></td>
				</c:forEach>
			</tr>
		</table>
	</c:if>
