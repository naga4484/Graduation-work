...
public class StuPasswordCodeVerifyAction extends Action {
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (sessionVerificationCode != null && sessionVerificationCode.equals(userInputCode)) {
            return "student_reset_password_form.jsp";  // 新しいパスワードを入力するページに遷移
        } else {
            request.setAttribute("errorMessage", "確認コードが正しくありません。");
            return "student_reset_password_code.jsp";  // 確認コード入力ページに戻る
        }
    }
}
